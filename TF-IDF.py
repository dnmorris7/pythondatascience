...
rawData = sc.textFile("d:/DataScience/subset-small.tsv")
fields = rawData.map(lambda x: x.split("\t")) #lambda is basically a return statement, so we don't have to def a subfunction here
documents = fields.map(lambda x: x[3].split(" ")) #by x[3] we mean that we're selecting the body of the text, and splits it into words

'''

#what about titles? Draft function by David Morris
titles = fields.map(lambda x: x[2].split(" "))  
'''
...
earthTF = hashingTF.transform(["earth"])
earthHashValue = int(earthTF.indices[0])

# Now we will extract the TF*IDF score for Gettsyburg's hash value into
# a new RDD for each document:
gettysburgRelevance = tfidf.map(lambda x: x[gettysburgHashValue])
earthRelevance = tfidf.map(lambda x: x[earthHashValue])
...
zippedResults = earthRelevance.zip(documentNames)

# And, print the document with the maximum TF*IDF value:
print "Best document for earth is:"
print zippedResults.max()
